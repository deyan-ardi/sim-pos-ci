...
class PhOSCo_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
     ...
     */
    protected $tags = [
        'author' => ['required' => true, 'allow_multiple' => true,
            'order_text'        => 'follows @subpackage (if used) or @package', ],
        'copyright' => ['required' => false, 'allow_multiple' => true, 'order_text' => 'follows @author'],
        'license'   => ['required' => true, 'allow_multiple' => false,
            'order_text'           => 'follows @copyright (if used) or @author', ],
        'version' => ['required' => false, 'allow_multiple' => false, 'order_text' => 'follows @license'],
        'see'     => ['required' => false, 'allow_multiple' => true, 'order_text' => 'follows @link'],
        'since'   => ['required'    => false, 'allow_multiple' => false,
            'order_text'            => 'follows @see (if used) or @link', ],
        'deprecated' => ['required' => false, 'allow_multiple' => false,
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $nextToken = [T_ABSTRACT, T_CLASS, T_FUNCTION, T_DOC_COMMENT];
    ...
    {
        $docBlock    = (static::class === 'PhOSCo_Sniffs_Commenting_FileCommentSniff') ? 'file' : 'class';
        ...
        foreach ($this->tags as $tag => $info) {
            ...
            if ($info['required'] === true && in_array($tag, $foundTags, true) === false) {
            ...
            if ((null === $tagElement) === true || empty($tagElement) === true) {
                continue;
            }
            ...
            // Get the tag order.
            $foundIndexes = array_keys($foundTags, $tag, true);
            ...
            if (is_array($tagElement) === true) {
                foreach ($tagElement as $key => $element) {
                    $indentation[] = ['tag' => $tag, 'space' => $this->getIndentation($tag, $element),
                        'line'              => $element->getLine(), ];
                }
            } else {
                $indentation[] = ['tag' => $tag, 'space' => $this->getIndentation($tag, $tagElement)];
            }
        ...
        foreach ($indentation as $indentInfo) {
            if ($indentInfo['space'] !== 0 && $indentInfo['space'] !== ($longestTag + 1)) {
                ...
                $data     = [$indentInfo['tag'], $expected, $space];
